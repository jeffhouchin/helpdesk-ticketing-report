...
class AzureAIService {
  constructor() {
    ...
    this.model = process.env.OPENAI_MODEL || 'gpt-4o-mini'; // Cheap and fast
  }
  ...
  async analyzeStuckTicket(ticket) {
    ...
    try {
      const response = await axios.post(`${this.openaiEndpoint}/chat/completions`, {
        model: this.model,
        messages: [
          {
            role: 'system',
            content: `You are a helpdesk management AI. Analyze tickets and provide actionable recommendations. 
    ...
    try {
      const response = await axios.post(`${this.openaiEndpoint}/chat/completions`, {
        ...
        messages: [
          {
            role: 'system',
            content: `You are a helpdesk performance evaluator. Grade technician performance on individual tickets.
      ...
      console.error('Performance grading failed:', error);
